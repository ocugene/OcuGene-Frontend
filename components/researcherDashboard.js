...
const ResearcherDashboard = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('https://ocugene-backend-1-production.up.railway.app/patient/count-per-region').then(response => {
        if (!response.ok) throw new Error('Failed to fetch regions data');
        return response.json();
      }),
      fetch('https://ocugene-backend-1-production.up.railway.app/patient/count-per-variant').then(response => {
